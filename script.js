...
function drawFilters() {
  const filters = {};
  for (const key of ["Area", "Shift", "Team", "Service", "Priority"]) {
    filters[key] = [...new Set(data.incidents.map((d) => d[key]))];
  }
  
  // List of services to pre-select
  ...
  // Modify the dropdown HTML generation to use a single row
  $filters.innerHTML = `
          ...
          <div class="flex-grow-1">
            <div class="dropdown">
              ...
              <div class="dropdown-menu w-100">
                ...
                <div class="dropdown-item" onclick="this.querySelector('input').click()">
                  <input type="checkbox" class="select-all" id="all-${key}" ${key !== 'Service' ? 'checked' : ''}>
                ...
                ${values
                  .map(v => `
                  <div class="dropdown-item" onclick="this.querySelector('input').click()">
                    <input type="checkbox" class="filter-checkbox" name="${key}" value="${v}" id="${key}-${v}" 
                      ${key === 'Service' ? (preSelectedServices.includes(v) ? 'checked' : '') : 'checked'}>
                    <label for="${key}-${v}" class="flex-grow-1">${v}</label>
                  </div>
                `)
                  .join("")}
              </div>
  ...
  document.querySelectorAll('.select-all').forEach(checkbox => {
    checkbox.addEventListener('change', (e) => {
      ...
      const dropdownMenu = e.target.closest('.dropdown-menu');
      dropdownMenu.querySelectorAll('.filter-checkbox').forEach(cb => {
  ...
  // Add event listeners for Top 10 checkbox
  document.querySelectorAll('.top-10').forEach(checkbox => {
    checkbox.addEventListener('change', (e) => {
      ...
      dropdownMenu.querySelectorAll('.filter-checkbox').forEach(cb => {
        cb.checked = e.target.checked && top10Services.includes(cb.value);
      });
      
      // Update the "Select All" checkbox state
      const selectAll = dropdownMenu.querySelector('.select-all');
...
function filteredIncidents() {
  return data.incidents.filter((row) => {
    return ["Area", "Shift", "Team", "Service","Priority"].every((key) => {
      const selectedValues = [...document.querySelectorAll(`input[name="${key}"]:checked`)].map(cb => cb.value);
      return selectedValues.length === 0 || selectedValues.includes(row[key]);
    });
  });
}

// Remove the existing filters event listener since we're now using checkboxes
// $filters.removeEventListener("change", update);
...
function drawSankey() {
  ...
  const graph = sankey($sankey, {
    ...
    labelWidth: 100,
    categories: ["Shift", "Area", "Team", "Service","Priority"],
    size: (d) =>  d.Count,
    text: (d) => {
      const textWidth = d.key.length * 9; // Approximate width of text (9 pixels per character)
